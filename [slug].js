...
export async function getServerSideProps({ params }) {
  try {
    const { data: battle, error } = await supabase
      .from("battles")
      .select(
        "id, slug, votes_a, votes_b, status, starts_at, ends_at, views, winner_id, product_a:product_a_id(id, name, slug, description, logo_url, website_url), product_b:product_b_id(id, name, slug, description, logo_url, website_url)"
      )
      .eq("slug", params.slug)
      .single();
