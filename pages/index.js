...
// Gold/silver/bronze for the Top Live Battles section. This is tied to
// CURRENT rank, not to any specific battle — since topBattles is
// recomputed fresh from live vote totals on every page load (see the
// ranking logic in getServerSideProps below), whichever battle happens
// to be #1 right now gets gold; if a different battle overtakes it
// later, the color moves with the rank, not a fixed hardcoded battle.
const TOP_BATTLE_MEDALS = [
  { label: "1ST", bg: "#E8B84B", text: "#4A3A0E" },
  { label: "2ND", bg: "#D9DBE0", text: "#3A3B44" },
  { label: "3RD", bg: "#E7B27B", text: "#4A2E10" },
];
    ...
    <div className="mx-auto max-w-6xl pb-12">
            ...
            {topBattles.map((b, i) => {
              ...
              return (
                <div key={b.id} className="relative pt-3">
                  <div
                    className="absolute left-3 top-0 z-10 flex items-center gap-1.5 rounded-full px-3 py-1 font-mono text-[10px] font-bold shadow-[2px_2px_0_#0B0C10]"
                  ...
                  <BattleCard battle={b} mode="compact" />
                </div>
              );
            })}
          </div>
      ...
      {happeningNow.length > 0 && (
        <section className="py-8">
          <div className="mb-3 flex items-baseline justify-between px-5 md:px-8">
          ...
          <div className="flex snap-x snap-mandatory gap-3 overflow-x-auto px-5 pb-2 md:px-8">
            {happeningNow.map((b) => (
              <div key={b.id} className="w-[260px] shrink-0 snap-start">
                <BattleCard battle={b} mode="compact" />
              </div>
            ))}
          </div>
