...
// ---------- category bar ----------
// Collapsed by default into a single "Categories" tab. Pressing it
// expands an inline panel of selectable pills below — still not a
// native <select> or a modal/popup, just a toggled section of the page.
function CategoryBar({ categories, activeCategorySlug, onSelect }) {
  ...
  function toggleOpen() {
    try {
      ...
      logError("pages/index.CategoryBar.toggleOpen", err);
    }
  }
  ...
  return (
    <section className="px-5 py-8 md:px-8">
      ...
      <form
        ...
        className="flex flex-col gap-3 rounded-xl border border-line bg-white p-4 md:max-w-2xl"
      >
