...
export async function getServerSideProps({ query }) {
  ...
  try {
    ...
    const { data: battles, error: battlesError } = await supabase
      .from("battles")
      .select(
        "id, slug, votes_a, votes_b, status, starts_at, ends_at, views, product_a:product_a_id(id, name, slug, logo_url), product_b:product_b_id(id, name, slug, logo_url)"
      )
      .eq("status", "live")
...
// ---------- start a battle ----------
// No "your product" framing anywhere — this app has no accounts, so
// nobody actually owns a product listing. Either slot can hold any
// existing product, filled in any order.
const DURATIONS = [
...
function StartBattlePanel({ categories }) {
  ...
  // Whichever side is filled first drives "recommended" suggestions for
  // the other, still-empty side.
  const recommendForA = !productA && productB ? productB : null;
  const recommendForB = !productB && productA ? productA : null;
      ...
      <div className="flex flex-col gap-4 sm:flex-row sm:items-start">
        <div className="flex-1">
          <ProductSlot
            ...
            categories={categories}
            recommendCategoryId={recommendForA?.category_id}
            excludeProductId={recommendForA?.id}
          />
        </div>
        ...
        <div className="flex-1">
          <ProductSlot
            ...
            categories={categories}
            recommendCategoryId={recommendForB?.category_id}
            excludeProductId={recommendForB?.id}
          />
        </div>
      </div>
...
// A single "pick a product" slot: search box + results, "+ Add a
// product" for creating a new one inline, and recommended-opponent chips
// based on the other (already-filled) slot's category, when this slot is
// still empty.
function ProductSlot({
  ...
  useEffect(() => {
    if (!recommendCategoryId || selectedProduct) {
      setRecommended([]);
      return;
    }
    let cancelled = false;
  ...
  }, [recommendCategoryId, excludeProductId, selectedProduct]);
...
// The "+ Add a product" mini form. Name, website, category, logo (PNG,
