...
export async function getServerSideProps({ params, req }) {
  try {
    const { data: battle, error } = await supabase
      .from("battles")
      .select(
        "id, slug, votes_a, votes_b, votes_a_boost, votes_b_boost, status, question, starts_at, ends_at, views, clicks, winner_id, created_by, product_a:product_a_id(id, name, slug, description, logo_url, website_url, category_id, category:category_id(name, slug, icon)), product_b:product_b_id(id, name, slug, description, logo_url, website_url, category_id)"
      )
      .eq("slug", params.slug)
      .single();
      ...
      if (relatedProductsError) {
        ...
        if (relatedIds.length > 0) {
          ...
          const { data: relatedBattles, error: relatedBattlesError } = await supabase
            .from("battles")
            .select(
              "id, slug, votes_a, votes_b, votes_a_boost, votes_b_boost, product_a:product_a_id(name, logo_url), product_b:product_b_id(name, logo_url)"
            )
            .or(orFilter)
            .neq("id", battle.id)
        ...
        <div className="mt-10 px-5 md:px-8">
          ...
          <div className="grid grid-cols-1 gap-3 sm:grid-cols-3">
            {related.map((b) => {
              ...
              const relTintA = getAvatarTint(b.product_a.name);
              const relTintB = getAvatarTint(b.product_b.name);
              return (
                <Link
                  key={b.id}
                  href={`/battle/${b.slug}`}
                  className="rounded-xl border border-line bg-white p-3 shadow-[2px_2px_0_#0B0C10] hover:border-cornerA"
                >
