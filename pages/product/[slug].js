...
export async function getServerSideProps({ params }) {
  try {
    ...
    const { data: battles, error: battlesError } = await supabase
      .from("battles")
      .select(
        "id, slug, status, votes_a, votes_b, views, winner_id, starts_at, ends_at, product_a_id, product_b_id, product_a:product_a_id(name, slug, logo_url), product_b:product_b_id(name, slug, logo_url)"
      )
      ...
      <div className="rounded-2xl border border-line bg-white p-5">
        <div className="flex items-start gap-4">
          ...
          <div className="min-w-0 flex-1">
            ...
            {product.category && (
              <div className="mt-1 inline-flex items-center gap-1 rounded-full border border-line bg-paper px-2.5 py-1 font-mono text-[10px] font-bold">
