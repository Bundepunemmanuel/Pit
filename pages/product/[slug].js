...
export default function ProductPage({ product, battles, rank }) {
  ...
  return (
    <div className="mx-auto max-w-2xl px-5 pb-10 pt-6 md:px-8 lg:max-w-3xl">
      {/* ---------- hero ---------- */}
      <div className="rounded-2xl border border-line bg-white p-5 shadow-[4px_4px_0_#0B0C10]">
        <div className="flex items-start gap-4">
          <div
            className="flex h-20 w-20 shrink-0 items-center justify-center overflow-hidden rounded-2xl border border-line font-display text-2xl md:h-24 md:w-24"
      ...
      {/* ---------- stats ---------- */}
      {/* Rating gets visual weight the other four don't — it's the
      headline stat (what actually determines rank), the rest are
      supporting context. Previously all five were identical boxes with
      no hierarchy at all. */}
      <div className="mt-4 grid grid-cols-3 gap-3 sm:grid-cols-5">
        <div className="flex flex-col items-center justify-center rounded-xl bg-ink py-3 text-center text-white shadow-[3px_3px_0_#FE4C12]">
        ...
        <div className="rounded-xl border border-line bg-white py-3 text-center shadow-[2px_2px_0_#0B0C10]">
        ...
        {battles.map((b) => {
          ...
          return (
            <Link
              key={b.id}
              href={`/battle/${b.slug}`}
              className="flex items-center gap-3 rounded-xl border border-line bg-white px-3 py-3 shadow-[2px_2px_0_#0B0C10] hover:border-cornerA"
            >
              <div
