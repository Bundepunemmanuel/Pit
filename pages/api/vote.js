...
import { calculateRatingChange, isMatchAllowed, MAX_RATING_GAP } from "../../elo";
...
async function createBattle(req, res) {
  try {
    const { productAId, productBId, duration } = req.body || {};
    ...
    // Cooldown — block re-matching the same pairing (in either order) if
    // they're already in a live battle or fought within the last 24h.
...
async function castVote(req, res) {
  try {
    ...
    // 4. Move ratings by the flat delta (see elo.js — no longer true Elo).
    const { newRatingA, newRatingB } = calculateRatingChange(
