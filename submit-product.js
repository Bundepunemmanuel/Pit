...
//
// This route now auto-publishes (status: "active") at the user's request.
// The original architecture doc recommended a "pending" review queue
// instead, specifically to stop junk submissions — worth knowing that
// protection is now off. If spam becomes a real problem, flipping the
// default status back to "pending" here is a one-line change.
...
export default async function handler(req, res) {
  ...
  if (!identifier || !identifier.trim()) {
    return res.status(400).json({ error: "A product URL or @handle is required" });
  }
  ...
  const isUrl = /^https?:\/\//.test(cleanIdentifier) || cleanIdentifier.includes(".");
  ...
  const { error: insertError } = await supabaseAdmin.from("products").insert({
    ...
    logo_url: logoUrl,
    website_url: isUrl ? cleanIdentifier.replace(/^(?!https?:\/\/)/, "https://") : null,
