...
export async function getServerSideProps({ query }) {
  ...
  try {
    ...
    const { data: battles, error: battlesError } = await supabase
      .from("battles")
      .select(
        "id, slug, votes_a, votes_b, status, question, starts_at, ends_at, views, winner_id, clicks, product_a:product_a_id(id, name, slug, logo_url, category:category_id(name, slug)), product_b:product_b_id(id, name, slug, logo_url, category:category_id(name, slug))"
      )
      .eq("status", "live")
    ...
    if (productsWithVotes.length > 0) {
      ...
      const { data: voteBattles, error: voteBattlesError } = await supabase
        .from("battles")
        .select("product_a_id, product_b_id, votes_a, votes_b")
        .or(
      ...
      if (voteBattlesError) {
        ...
        for (const b of voteBattles ?? []) {
          if (productIds.includes(b.product_a_id)) {
            totals[b.product_a_id] = (totals[b.product_a_id] ?? 0) + (b.votes_a ?? 0);
          }
          if (productIds.includes(b.product_b_id)) {
            totals[b.product_b_id] = (totals[b.product_b_id] ?? 0) + (b.votes_b ?? 0);
          }
        }
            ...
            {trending.map((b) => {
              ...
              return (
                ...
                >
                  ...
                  {category?.slug && (
                    <div className="mt-1.5 flex items-center justify-center gap-1 font-mono text-[10px] text-grayText">
                      <CategoryIcon slug={category.slug} className="h-3 w-3 shrink-0" />
                      {category.name}
                    </div>
                  )}
                  <div className="mt-1 flex items-center justify-center gap-1 font-mono text-[10px] text-grayText">
                    <span className="h-1.5 w-1.5 rounded-full bg-cornerA" />
                    LIVE · {total.toLocaleString()} votes
                    {typeof b.clicks === "number" && ` · ${b.clicks.toLocaleString()} clicks`}
                  </div>
                </Link>
              );
            })}
